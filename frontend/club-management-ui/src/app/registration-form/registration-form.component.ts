  ...
  userRegistration() {
    ...
    const handleRollNumberSubmission = () => {
      ...
      if (regex.test(this.rollnumber)) {
        ...
        fetch('/otp/send', {
          ...
          body: JSON.stringify({ roll: this.rollnumber })
        })
    ...
    const handleOtpSubmission = () => {
      ...
      if (otp) {
        ...
        fetch('/otp/verify', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ roll: this.rollnumber, otp: otp })
        })
          .then(response => response.json())
          .then(data => {
            if (data.success) {
    ...
    const handlePasswordCreation = () => {
      ...
      if (password && confirmPassword && password === confirmPassword) {
        this.isButtonDisabled = true;
        fetch('/user/create-password', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ roll: this.rollnumber, password: password })
        })
          .then(response => response.json())
          .then(data => {
            if (data.success) {
